    ...
    <div class="grid grid-cols-3 mx-auto max-w-7xl ">
      <div class="py-6 px-6 lg:px-8 bg-white m-4 rounded-xl shadow col-span-3 lg:col-span-2">
        <div class="mx-auto max-w-2xl text-center">
          ...
          <div class="mt-4">
            <input v-model="params.search" type="text" name="search" id="search" class="block w-full rounded-md border-gray-300 shadow-sm focus:border-primary-500 focus:ring-primary-500 sm:text-sm" placeholder="Search some Frequentle Ask Questions" />
          </div>
        ...
        <div class="mt-6 col-span-3 lg:col-span-1">
          <dl class="sm:grid sm:grid-cols-1 sm:gap-x-6 sm:gap-y-4 sm:space-y-4">
            <div v-for="row in content" :key="row.id">
...
async function GetAPI() {
  config.loading = true
  try {
    let { data: {data} } = await axios.get('/api/faq-public', { params: params})
    content.value = data
