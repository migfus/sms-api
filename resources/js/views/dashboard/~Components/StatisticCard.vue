      ...
      <div v-for="item in stats" :key="item.name" class="px-4 py-5 sm:p-6">
        ...
        <dd class="mt-1 flex items-baseline justify-between md:block lg:flex">
          ...
          <div :class="[item.changeType === 'increase' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800', 'inline-flex items-baseline px-2.5 py-0.5 rounded-full text-sm font-medium md:mt-2 lg:mt-0']">
            <ArrowUpIcon v-if="item.changeType === 'increase'" class="mr-0.5 h-5 w-5 flex-shrink-0 self-center text-green-500" aria-hidden="true" />
            <ArrowDownIcon v-else class="mr-0.5 h-5 w-5 flex-shrink-0 self-center text-red-500" aria-hidden="true" />
