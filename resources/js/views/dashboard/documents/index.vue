<template>
  ...
  <div v-if="$doc.content" class="bg-white rounded-b-xl shadow">
    <div class="px-4 pb-5 sm:p-6 sm:pt-0">

      <ul role="list" class="grid grid-cols-2 gap-x-4 gap-y-8 sm:grid-cols-3 sm:gap-x-6 lg:grid-cols-5 xl:grid-cols-7 xl:gap-x-8">
        <DataTransition>
          <IndividualFile v-for="row in $doc.content" :key="row.id" :file="row" />
        </DataTransition>
      </ul>

      <div v-if="$doc.content.length == 0" class="text-center">
        <div class="empty-icon-container">
          <div class="animation-container opacity-40">
            <div class="bounce" :style="`background-image: url(${tumble});`"></div>
            <div class="pebble1"></div>
            <div class="pebble2"></div>
            <div class="pebble3"></div>
          </div>
          <div>
            <p class="mt-3 text-sm text-gray-500">Seems empty in here.</p>
          </div>
        </div>
...
.pebble3{
  ...
  height:3px;
  border-radius: 20px 20px 0px 0px;
  animation: pebblesAnim 6s linear infinite;
}
